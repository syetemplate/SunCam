...
const validationSchema = yup.object().shape({
    name: yup.string().min(2, content.checkout.labels.nameError).required(content.checkout.labels.nameError),
    phone: yup.string().min(7, content.checkout.labels.phoneError).required(content.checkout.labels.phoneError),
    email: yup.string().email(content.checkout.labels.emailError).required(content.checkout.labels.emailError),
});
...
const Checkout = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async values => {
            try {
                const response = await axios.post('/api/submit-details', values);
                ...
                {!postSubmitMessage && (
                    <FormikProvider value={formik}>
                        <form
                            ...
                            className="flex w-full flex-col lg:ml-0 lg:mr-4 rtl:lg:ml-12 rtl:lg:mr-0"
                        >
                            ...
                            <Field name="name" type="text">
                                {({ field, meta }) => (
                                    <>
                                        <input
                                            className="mt-6 focus:ring-2 focus:ring-gray-500 focus:outline-none px-2 border-b border-gray-200 leading-4 text-sm placeholder-gray-600 py-4 w-full lg:w-[95%] placeholder:text-left rtl:placeholder:text-right"
                                            placeholder={content.checkout.labels.name}
                                            disabled={formik.isSubmitting}
                                            {...field}
                                        />
                                        {meta.touched && meta.error && (
                                            <p className="text-red-500 text-sm mt-2 mb-0">{meta.error}</p>
                                        )}
                                    </>
                                )}
                            </Field>
